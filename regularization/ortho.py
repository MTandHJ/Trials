...
assert freerec.__version__ == "0.0.16", "Version Not Match, 0.0.16 required ..."
...
class PurEmbd(RecSysArch):
    ...
    def regularize(self, users: Dict[str, torch.Tensor], items: Dict[str, torch.Tensor], rtype: str = 'l2') -> Dict[str, torch.Tensor]:
        ...
        if rtype == 'l2':
            ...
            def ortho_regularizer(weight: torch.Tensor):
                weight = F.normalize(weight, dim=0) # n x d
            ...
            loss = 0
            loss += ortho_regularizer(userEmbs)
            loss += ortho_regularizer(itemEmbs)
        return loss
